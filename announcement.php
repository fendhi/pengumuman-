...
<head>
  ...
  <title><?= h($announcement['title']) ?> - <?= h(APP_NAME) ?></title>
  <?= tailwind_cdn_script() ?>
</head>
      ...
      <div class="mt-6 grid grid-cols-1 gap-6 lg:grid-cols-12">
        <div class="lg:col-span-4">
          <div class="rounded-xl border bg-slate-50 p-5">
            ...
            <?php endif; ?>
            <div class="mt-4 text-xs text-slate-500">Tautan publik: pengumuman ini bisa dibagikan dari admin.</div>
          </div>
        </div>
            ...
            <div class="border-b bg-slate-50 px-4 py-3">
              <div class="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                <div class="min-w-0">
                  ...
                  <div class="mt-0.5 text-xs text-slate-500">Menampilkan <?= (int)$shownParticipants ?> dari <?= (int)$totalParticipants ?> peserta<?= $q !== '' ? ' (hasil pencarian)' : '' ?>.</div>
                </div>
                ...
                <form class="flex w-full items-center gap-2 sm:w-auto" method="get" action="">
                  ...
                  <?php if ($q !== ''): ?>
                    <a class="rounded-md border bg-white px-3 py-2 text-sm font-semibold hover:bg-slate-50" href="<?= h(BASE_URL) ?>/announcement.php?id=<?= (int)$id ?>">Reset</a>
                  <?php endif; ?>
                </form>
              </div>
            </div>
